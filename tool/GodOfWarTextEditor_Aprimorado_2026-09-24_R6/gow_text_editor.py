...
if QT_AVAILABLE:
    ...
    class App(QMainWindow):
        ...
        def _build_ui(self):
            ...
            # Em MSGS_TXT, mover o cursor para outra linha atualiza o vínculo
            # MessageTemplate_LineN sem obrigar o usuário a abrir outro diálogo.
            self.editor.cursorPositionChanged.connect(self._on_editor_cursor_changed)
            editor_v.addWidget(self.editor, 1)

            self.inline_colors_box = QGroupBox("CORES ASSOCIADAS AO TEXTO ATIVO")
            inline_v = QVBoxLayout(self.inline_colors_box)
            inline_v.setContentsMargins(int(7 * self._ui_scale), int(5 * self._ui_scale),
                                        int(7 * self._ui_scale), int(5 * self._ui_scale))
            inline_v.setSpacing(int(4 * self._ui_scale))
            self.inline_colors_summary = QLabel(
                "Selecione um texto. As cores de FLP GoW2 relacionadas aparecerão aqui."
            )
            self.inline_colors_summary.setWordWrap(True)
            self.inline_colors_summary.setObjectName("MutedLabel")
            inline_v.addWidget(self.inline_colors_summary)
            self.inline_colors_table = QTableWidget(0, 6)
            self.inline_colors_table.setHorizontalHeaderLabels(
                ["FLP", "Camada", "Campo / linha", "Cor RGBA", "Alfa", "Alcance"]
            )
            self.inline_colors_table.setSelectionBehavior(QAbstractItemView.SelectionBehavior.SelectRows)
            self.inline_colors_table.setSelectionMode(QAbstractItemView.SelectionMode.SingleSelection)
            self.inline_colors_table.setEditTriggers(QAbstractItemView.EditTrigger.NoEditTriggers)
            self.inline_colors_table.setAlternatingRowColors(True)
            self.inline_colors_table.setShowGrid(False)
            self.inline_colors_table.verticalHeader().setVisible(False)
            self.inline_colors_table.verticalHeader().setDefaultSectionSize(int(21 * self._ui_scale))
            self.inline_colors_table.horizontalHeader().setStretchLastSection(True)
            self.inline_colors_table.setMinimumHeight(int(110 * self._ui_scale))
            self.inline_colors_table.setMaximumHeight(int(156 * self._ui_scale))
            self.inline_colors_table.setColumnWidth(0, int(82 * self._ui_scale))
            self.inline_colors_table.setColumnWidth(1, int(98 * self._ui_scale))
            self.inline_colors_table.setColumnWidth(2, int(230 * self._ui_scale))
            self.inline_colors_table.setColumnWidth(3, int(126 * self._ui_scale))
            self.inline_colors_table.setColumnWidth(4, int(48 * self._ui_scale))
            self.inline_colors_table.itemSelectionChanged.connect(self._on_inline_color_select)
            inline_v.addWidget(self.inline_colors_table)
            inline_bottom = QHBoxLayout()
            self.inline_colors_detail = QLabel("Selecione uma cor para ver seu alcance.")
            self.inline_colors_detail.setWordWrap(True)
            self.inline_colors_detail.setObjectName("MutedLabel")
            inline_bottom.addWidget(self.inline_colors_detail, 1)
            self.inline_colors_edit_button = QPushButton("Editar cor selecionada…")
            self.inline_colors_edit_button.setObjectName("MiniButton")
            self.inline_colors_edit_button.setEnabled(False)
            self.inline_colors_edit_button.clicked.connect(self._edit_inline_selected_color)
            inline_bottom.addWidget(self.inline_colors_edit_button)
            inline_v.addLayout(inline_bottom)
            editor_v.addWidget(self.inline_colors_box)
        ...
        def _on_editor_cursor_changed(self):
            if self._loading or self.active_tag is None:
                return
            # Só MSGS_TXT possui mapeamento por linha/cursor. StaticLabels têm
            # associação por rótulo e não devem redesenhar a tabela a cada seta.
            if self.active_tag.upper_name == "MSGS_TXT" and not self._is_main_flp_mode():
                self._refresh_inline_text_colors()
