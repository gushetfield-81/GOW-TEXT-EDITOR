...
def wrap_inline_style_selection(text: str, start: int, end: int, style: int) -> str:
    """Envolve seleção; quando cruza linha, cria um par por linha real."""
    style = int(style)
    ...
    if "\n" not in selected and "\r" not in selected:
        wrapped = marker + selected + "[*0]"
    else:
        parts = re.split(r"(\r\n|\r|\n)", selected)
        wrapped = "".join(part if part in {"\r", "\n", "\r\n"} else marker + part + "[*0]" for part in parts)
...
if QT_AVAILABLE:
    ...
    class App(QMainWindow):
        ...
        def _build_ui(self):
            ...
            self.inline_style_tag = QLabel("COR INLINE")
            ...
            self.inline_style_selector.setMinimumContentsLength(18)
            ...
            self.inline_style_selector.setToolTip(
                "Controle literal do renderer Flash. [*0]/[*] volta à cor base; "
                "[*1]..[*4] vale até o fim da mesma linha.")
            ...
            self.inline_style_apply_button = QPushButton("Inserir [*N]")
            self.inline_style_apply_button.setObjectName("MiniButton")
            self.inline_style_apply_button.setToolTip(
                "Insere/troca o token. Com seleção multilinha, usa um par por linha e preserva [Icon...].")
            ...
            self.inline_style_palette_button = QPushButton("Editar RGB runtime…")
            self.inline_style_palette_button.setObjectName("MiniButton")
            self.inline_style_palette_button.setToolTip(
                "Altera somente os três floats RGB de GBL_Global.FlashMsgNColor.")
            ...
            self.inline_style_hint = QLabel("Abra MSGS_TXT para interpretar [*N] pelo runtime.")
        ...
        def _refresh_inline_style_controls(self, preferred_style: int | None = None):
            ...
            with QSignalBlocker(self.inline_style_selector):
                ...
                if enabled:
                    self.inline_style_selector.addItem("[*0] / [*] • cor base da linha", 0)
                    for style in INLINE_RENDERED_STYLE_IDS:
                        rgba = palette.color(style)
                        self.inline_style_selector.addItem(
                            f"[*{style}] • FlashMsg{style}Color • {self._inline_rgb_hex(rgba)}", style)
                        ...
                        if rgba is not None:
                            ...
                            self.inline_style_selector.setItemData(row, QColor("#111114") if light > 145 else QColor("#f2f2f6"), Qt.ForegroundRole)
                    row = next((i for i in range(self.inline_style_selector.count())
                                if self.inline_style_selector.itemData(i) == preferred_style), 1)
            ...
            self.inline_style_palette_button.setEnabled(enabled and preferred_style in INLINE_RENDERED_STYLE_IDS and palette.editable)
            if not enabled:
                hint = "Abra uma mensagem MSGS_TXT para interpretar [*N] no renderer Flash."
            elif palette.editable:
                hint = f"{palette.source} • [*N] substitui a cor base; RGB editável no Data Context."
            else:
                hint = f"{palette.source} • visualização; este WAD não possui campo RGB editável."
        ...
        def _sync_inline_style_from_cursor(self):
            if not self._is_inline_runtime_message_mode():
                return
            match = inline_style_control_at(self.editor.toPlainText(), self.editor.textCursor().position())
            if match is not None:
                style = 0 if match.group("style") is None else int(match.group("style"))
        ...
        def _apply_inline_style_control(self):
            ...
            if match is not None:
                ...
                status = f"Seleção envolvida por [*{style}] … [*0] em {lines} linha(s)."
            else:
                cursor.insertText(f"[*{style}]")
        ...
        def _edit_inline_runtime_palette(self):
            ...
            palette = self.runtime_inline_palette
            if style not in INLINE_RENDERED_STYLE_IDS:
                QMessageBox.information(self, "Cor inline", "[*0] e [*] usam a cor base; escolha [*1] até [*4].")
                return
            if not palette.editable or self.wad is None or palette.data_tag is None:
                QMessageBox.information(self, "Cor inline", "Este WAD não contém GBL_Global editável para a paleta [*N].")
                return
            rgba = palette.color(style)
            if rgba is None:
                QMessageBox.warning(self, "Cor inline", "FlashMsgColor não foi encontrada.")
                return
            chosen = QColorDialog.getColor(QColor(*rgba), self,
                                            f"Editar FlashMsg{style}Color (runtime [*{style}])")
            ...
            try:
                ...
                self._refresh_inline_style_controls(style)
                self.update_preview()
                self._set_status(f"FlashMsg{style}Color = #{wanted[0]:02X}{wanted[1]:02X}{wanted[2]:02X}; salve o WAD para gravar.")
            except Exception as exc:
                QMessageBox.critical(self, "Não foi possível editar a paleta inline", str(exc))
