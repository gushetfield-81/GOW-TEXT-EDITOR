...
# ---------------------------------------------------------------------------
# Controles de estilo inline do Flash PS2: [*N]
# ---------------------------------------------------------------------------
# O runtime EditTextBuild consome [*N] e atribui o estilo aos TextItems sem
# produzir glifos para o token. DoMsgPage entrega cada linha a uma instância
# MessageTemplate_LineN distinta, portanto style reinicia em zero em cada linha.
# A cor vem de GBL_Global.m_FlashMsgNColor; nunca de HTML ou de uma paleta
# inventada pelo editor.
INLINE_STYLE_MARKER_RE = re.compile(r"\[\*(?P<style>[0-9])?\]")
INLINE_RENDERED_STYLE_IDS = (1, 2, 3, 4)
DC_DATA_IFF_TAG = 0x000C
DC_EXPORT_IFF_TAG = 0x000D
DC_GLOBAL_SYMBOL = "GBL_Global"
TGLOBAL_FLASH_MSG1_COLOR_OFFSET = 0x410
TGLOBAL_FLASH_MSG_COLOR_COUNT = 6
TGLOBAL_FLASH_MSG_COLOR_SIZE = 0x10
VERIFIED_R_PERMA_INLINE_RGB = {
    1: (127, 40, 5),
    2: (170, 89, 20),
    3: (102, 102, 102),
    4: (102, 153, 204),
...
class InlineStyleControl:
    """Token bruto [*N] visível no editor e consumido pelo renderer."""
    block: int
    start: int
    end: int
    raw: str
    style: int | None
    supported_by_renderer: bool


@dataclass
class InlineStyleRenderPlan:
    """Plano visual puro; jamais altera o texto que será serializado."""
    spans: dict[int, list[tuple[int, int, tuple[int, int, int, int]]]]
    controls: dict[int, list[InlineStyleControl]]
    unsupported_controls: list[InlineStyleControl]


@dataclass
class RuntimeInlinePalette:
    """FlashMsg colors extraídas de GBL_Global ou referência só de leitura."""
    colors: dict[int, tuple[int, int, int, int]]
    raw_floats: dict[int, tuple[float, float, float, float]]
    source: str
    data_tag: "WadTag | None" = None
    global_offset: int | None = None

    @property
    def editable(self) -> bool:
        return self.data_tag is not None and self.global_offset is not None

    def color(self, style: int) -> tuple[int, int, int, int] | None:
        return self.colors.get(int(style))

    def field_offset(self, style: int) -> int:
        if not self.editable or not 1 <= int(style) <= TGLOBAL_FLASH_MSG_COLOR_COUNT:
            raise ValueError("A cor FlashMsg solicitada não possui campo editável no GBL_Global atual.")
        assert self.global_offset is not None
        return self.global_offset + TGLOBAL_FLASH_MSG1_COLOR_OFFSET + (int(style) - 1) * TGLOBAL_FLASH_MSG_COLOR_SIZE

    @classmethod
    def verified_shared_reference(cls) -> "RuntimeInlinePalette":
        return cls(
            colors={style: (*rgb, 255) for style, rgb in VERIFIED_R_PERMA_INLINE_RGB.items()},
            raw_floats={},
            source="referência compartilhada de R_PERMA/GBL_Global (somente visualização)",
        )


def _runtime_flash_byte(component: float) -> int:
    """Replica a quantização do runtime: uint8(int(float * 255))."""
    if not math.isfinite(component):
        raise ValueError("Componente FlashMsg não é finito.")
    value = int(component * 255.0) if component > 0.0 else 0
    return max(0, min(255, value))


def _dc_export_offset(data_tag: "WadTag", export_tag: "WadTag", symbol: str) -> int | None:
    """Resolve uma exportação IFF (offset Data, offset nome) sem reserializar."""
    table = export_tag.data
    if len(table) < 4:
        return None
    count = struct.unpack_from("<I", table, 0)[0]
    pair_end = 4 + count * 8
    if count > 16384 or pair_end > len(table):
        return None
    wanted = symbol.encode("ascii")
    for index in range(count):
        data_offset, name_offset = struct.unpack_from("<II", table, 4 + index * 8)
        if name_offset >= len(table):
            continue
        name_end = table.find(b"\0", name_offset)
        if name_end < 0 or table[name_offset:name_end] != wanted:
            continue
        if data_offset + TGLOBAL_FLASH_MSG1_COLOR_OFFSET + TGLOBAL_FLASH_MSG_COLOR_SIZE > len(data_tag.data):
            return None
        return data_offset
    return None


def find_runtime_inline_palette(wad: "WadFile") -> RuntimeInlinePalette | None:
    """Lê FlashMsg1..6Color no GBL_Global do Data Context do WAD."""
    for data_tag in wad.tags:
        if data_tag.tag != DC_DATA_IFF_TAG:
            continue
        exports = [candidate for candidate in wad.tags
                   if candidate.tag == DC_EXPORT_IFF_TAG and candidate.name_raw == data_tag.name_raw]
        for export_tag in exports:
            global_offset = _dc_export_offset(data_tag, export_tag, DC_GLOBAL_SYMBOL)
            if global_offset is None:
                continue
            colors: dict[int, tuple[int, int, int, int]] = {}
            raw_floats: dict[int, tuple[float, float, float, float]] = {}
            try:
                for style in range(1, TGLOBAL_FLASH_MSG_COLOR_COUNT + 1):
                    offset = global_offset + TGLOBAL_FLASH_MSG1_COLOR_OFFSET + (style - 1) * TGLOBAL_FLASH_MSG_COLOR_SIZE
                    rgba_float = struct.unpack_from("<ffff", data_tag.data, offset)
                    if not all(math.isfinite(value) for value in rgba_float):
                        raise ValueError("float não finito")
                    raw_floats[style] = rgba_float
                    colors[style] = (
                        _runtime_flash_byte(rgba_float[0]),
                        _runtime_flash_byte(rgba_float[1]),
                        _runtime_flash_byte(rgba_float[2]),
                        255,  # alfa inline vem do cxform pai, não deste campo
                    )
            except (ValueError, struct.error):
                continue
            return RuntimeInlinePalette(colors=colors, raw_floats=raw_floats,
                source=f"{data_tag.name} → {DC_GLOBAL_SYMBOL}", data_tag=data_tag,
                global_offset=global_offset)
    return None


def runtime_inline_palette_for_wad(wad: "WadFile | None") -> RuntimeInlinePalette:
    if wad is not None:
        palette = find_runtime_inline_palette(wad)
        if palette is not None:
            return palette
    return RuntimeInlinePalette.verified_shared_reference()


def patched_runtime_inline_palette_rgb(palette: RuntimeInlinePalette, style: int,
                                       rgb: tuple[int, int, int]) -> bytes:
    """Cria patch de somente três floats RGB de FlashMsgNColor."""
    if not palette.editable or palette.data_tag is None:
        raise ValueError("Este WAD não contém GBL_Global; a paleta exibida é somente referência do runtime.")
    style = int(style)
    if style not in INLINE_RENDERED_STYLE_IDS:
        raise ValueError("O renderer analisado constrói apenas os estilos inline [*1] até [*4].")
    try:
        red, green, blue = (int(value) for value in rgb)
    except (TypeError, ValueError) as exc:
        raise ValueError("A cor inline precisa ter RGB inteiro.") from exc
    if any(value < 0 or value > 255 for value in (red, green, blue)):
        raise ValueError("Cada canal RGB deve estar entre 0 e 255.")
    offset = palette.field_offset(style)
    before = palette.data_tag.data
    if offset + 12 > len(before):
        raise ValueError("Campo FlashMsg RGB fora do Data Context.")
    expected = palette.raw_floats.get(style)
    if expected is not None and struct.unpack_from("<ffff", before, offset) != expected:
        raise ValueError("A paleta mudou desde a leitura; reabra o WAD antes de editar a cor inline.")
    def to_runtime_float(channel: int) -> float:
        return 1.0 if channel == 255 else (channel + 0.25) / 255.0
    patched = bytearray(before)
    struct.pack_into("<fff", patched, offset, *(to_runtime_float(value) for value in (red, green, blue)))
    candidate = bytes(patched)
    if candidate[:offset] != before[:offset] or candidate[offset + 12:] != before[offset + 12:]:
        raise ValueError("Validação interna: bytes fora do RGB FlashMsg seriam alterados.")
    return candidate


def build_inline_style_render_plan(text: str, palette: RuntimeInlinePalette) -> InlineStyleRenderPlan:
    """Interpreta [*N]/[*] por linha, como chamadas separadas de EditTextBuild."""
    normalized = text.replace("\r\n", "\n").replace("\r", "\n")
    spans: dict[int, list[tuple[int, int, tuple[int, int, int, int]]]] = {}
    controls: dict[int, list[InlineStyleControl]] = {}
    unsupported: list[InlineStyleControl] = []
    for block, line in enumerate(normalized.split("\n")):
        active_style = 0
        run_start = 0
        line_spans: list[tuple[int, int, tuple[int, int, int, int]]] = []
        line_controls: list[InlineStyleControl] = []
        for match in INLINE_STYLE_MARKER_RE.finditer(line):
            if match.start() > run_start and active_style in INLINE_RENDERED_STYLE_IDS:
                rgba = palette.color(active_style)
                if rgba is not None:
                    line_spans.append((run_start, match.start() - run_start, rgba))
            raw_style = match.group("style")
            style = 0 if raw_style is None else int(raw_style)
            supported = style == 0 or style in INLINE_RENDERED_STYLE_IDS
            control = InlineStyleControl(block, match.start(), match.end(), match.group(0), style, supported)
            line_controls.append(control)
            if not supported:
                unsupported.append(control)
            active_style = style
            run_start = match.end()
        if run_start < len(line) and active_style in INLINE_RENDERED_STYLE_IDS:
            rgba = palette.color(active_style)
            if rgba is not None:
                line_spans.append((run_start, len(line) - run_start, rgba))
        if line_spans:
            spans[block] = line_spans
        if line_controls:
            controls[block] = line_controls
    return InlineStyleRenderPlan(spans=spans, controls=controls, unsupported_controls=unsupported)


def inline_style_control_at(text: str, position: int) -> re.Match[str] | None:
    for match in INLINE_STYLE_MARKER_RE.finditer(text):
        if match.start() <= position < match.end():
            return match
    return None


def inline_style_at_position(text: str, position: int) -> int:
    """Resolve o estilo Flash ativo no cursor, respeitando o reinício por linha.

    ``EditTextBuild`` é chamado separadamente para cada linha entregue a
    ``MessageTemplate_LineN``.  Por isso um marcador de uma linha nunca vaza
    para a seguinte, mesmo quando o cursor usa offsets absolutos do editor.
    Se o cursor estiver dentro de ``[*N]``, o próprio controle é a rota que
    ele passa a selecionar; isso torna o token editável sem uma suposição
    sobre o glifo anterior.
    """
    if not text:
        return 0
    position = max(0, min(int(position), len(text)))
    # QPlainTextEdit normaliza para \n, mas manter \r aqui deixa o helper
    # seguro para os testes e para dados ainda não inseridos no documento.
    line_start = max(text.rfind("\n", 0, position), text.rfind("\r", 0, position)) + 1
    line_end_candidates = [value for value in (text.find("\n", position), text.find("\r", position)) if value >= 0]
    ...
    for match in INLINE_STYLE_MARKER_RE.finditer(line):
        raw_style = match.group("style")
        style = 0 if raw_style is None else int(raw_style)
        # Cursor dentro do literal [*N] seleciona o estilo que ele declara.
        if match.start() <= local_position < match.end():
            return style
        if match.end() <= local_position:
            active_style = style
            continue
        break
    return active_style
...
def inline_styles_in_range(text: str, start: int, end: int) -> set[int]:
    """Retorna as rotas ``[*N]`` que alcançam uma seleção real do editor.

    A rotina é deliberadamente pequena e baseada em offsets de caracteres: a
    caixa de mensagens tem poucos KB, e iterar os glifos evita declarar uma
    seleção mista como se tivesse uma única cor física. Marcadores sem glifo
    também entram no resultado, pois são selecionáveis e serializáveis.
    """
    start, end = sorted((max(0, int(start)), max(0, int(end))))
    start, end = min(start, len(text)), min(end, len(text))
    if start == end:
        return {inline_style_at_position(text, start)}
    routes: set[int] = set()
    for position in range(start, end):
        if text[position] in "\r\n":
            continue
        routes.add(inline_style_at_position(text, position))
    return routes or {inline_style_at_position(text, start)}


def replace_inline_style_at(text: str, position: int, style: int) -> str | None:
    style = int(style)
    if style not in range(0, 10):
        raise ValueError("O marcador inline precisa usar um único dígito.")
    match = inline_style_control_at(text, position)
    if match is None:
        return None
    return text[:match.start()] + f"[*{style}]" + text[match.end():]


def wrap_inline_style_selection(text: str, start: int, end: int, style: int) -> str:
    """Envolve só a seleção e restaura a rota que havia depois de cada trecho.

    O renderer mantém um estilo ativo até outro token ou o fim da linha. Uma
    implementação que fecha sempre com ``[*0]`` altera silenciosamente o texto
    *depois* da seleção quando ela estava dentro de um trecho ``[*N]``.  Este
    helper grava um fechamento serializável com o estilo original no limite
    de cada linha, preservando tanto tokens existentes quanto o restante da
    mensagem.
    """
    style = int(style)
    if style not in INLINE_RENDERED_STYLE_IDS and style != 0:
        raise ValueError("A interface expõe somente estilos comprovados [*0]..[*4].")
    start, end = sorted((max(0, int(start)), max(0, int(end))))
    start, end = min(start, len(text)), min(end, len(text))
    marker = f"[*{style}]"
    if start == end:
    ...
    position = start
    while position < end:
        # Não normalizar as quebras: o TXT original pode usar CRLF e o
        # Message.set_text preservará a convenção ao serializar o recurso.
        newline_at = len(text)
        newline_size = 0
        for candidate, size in ((text.find("\r\n", position, end), 2),
                                (text.find("\n", position, end), 1),
                                (text.find("\r", position, end), 1)):
            if candidate >= 0 and candidate < newline_at:
                newline_at, newline_size = candidate, size
        segment_end = min(end, newline_at)
        if segment_end > position:
            original_after = inline_style_at_position(text, segment_end)
            pieces.append(marker)
            pieces.append(text[position:segment_end])
            pieces.append(f"[*{original_after}]")
        if newline_at >= end:
            break
        pieces.append(text[newline_at:newline_at + newline_size])
        position = newline_at + newline_size
...
class FLPMovie:
    ...
    def colors_for_text_target(self, target_kind: str, target_index: int) -> dict:
        ...
        return {
            ...
            "runtime_paths": self.runtime_paths_for_text_target(kind, index),
        }

    def runtime_paths_for_text_target(self, target_kind: str, target_index: int) -> tuple[str, ...]:
        """Encontra caminhos de cena FLP que realmente alcançam um alvo de texto.

        DynamicLabels com o mesmo nome não são automaticamente a mesma
        instância runtime. Em ``FLP_HUDA`` por exemplo, as cópias de
        ``MessageTemplate_Line1`` pertencem a ramos distintos da raiz
        (``MessageTemplates`` e ``PickUpInfoMenu``). Atravessar KeyFrames de
        volta à raiz torna essa diferença visível sem adivinhar que cor o jogo
        escolheu num evento que só existe no runtime.
        """
        self._require_gow2_colors()
        aliases = {
            "dynamic": "dynamic_label", "dynamiclabel": "dynamic_label",
        ...
        try:
            ...
            raise ValueError("Índice de alvo de texto inválido.") from exc
        cache = getattr(self, "_runtime_text_paths_cache", None)
        ...
        layout = self._color_layout()
        kind_from_handler = {
            4: "static_label", 5: "dynamic_label", 6: "data6", 7: "data7", 8: "data8",
        }
        parents: dict[tuple[str, int], list[tuple[tuple[str, int], str]]] = {}
        for frame in layout["keyframes"]:
            handler = frame["handler"]
            if not 0 <= handler < len(self.gh):
                continue
            handler_kind, handler_index = self.gh[handler]
            child_kind = kind_from_handler.get(handler_kind)
            if child_kind is None:
                continue
            child = (child_kind, handler_index)
            parent = (frame["source_kind"], frame["source_index"])
            label = self._string_from_sector(
                self.data, layout["strings_pos"], layout["strings_size"], frame["name_offset"]
            )
            edge = (parent, label)
            if edge not in parents.setdefault(child, []):
                parents[child].append(edge)

        resolved: set[str] = set()
        max_paths = 64

        def climb(node: tuple[str, int], labels: list[str], seen: set[tuple[str, int]], depth: int):
            if len(resolved) >= max_paths or depth > 32:
                return
            if node == ("data8", 0):
                # O percurso é filho -> raiz; rótulos são exibidos raiz -> filho.
                meaningful = [item for item in reversed(labels) if item]
                resolved.add(" → ".join(meaningful) if meaningful else "raiz FLP (sem rótulo)")
                return
            edges = parents.get(node, [])
            if not edges:
                if labels:
                    meaningful = [item for item in reversed(labels) if item]
                    resolved.add(" → ".join(meaningful) if meaningful else "ramo FLP sem rótulo")
                return
            for parent, label in edges:
                if parent in seen:
                    continue
                climb(parent, labels + ([label] if label else []), seen | {parent}, depth + 1)

        climb((kind, index), [], {(kind, index)}, 0)
        result = tuple(sorted(resolved))
        if cache is None:
            cache = {}
            self._runtime_text_paths_cache = cache
...
if QT_AVAILABLE:
    ...
    class PreviewWidget(QWidget):
        """Prévia CRT: cores físicas base + estilos [*N] do runtime Flash."""
        STAGE_W = 800.0
        STAGE_H = 600.0
        X_LEFT = 2.5
        BOTTOM_OFF = 105.75
        SAFE = 30.0
        LINE_H = 21.0
        GAPS = (31.25, 37.5, 43.75)

        def _box_w(self):
            return 480.0 if self.ratio else 440.0
        ...
        def __init__(self, parent=None):
            ...
            self.inline_style_plan: InlineStyleRenderPlan | None = None
            self.setMinimumHeight(190)
            self.setAutoFillBackground(False)

        def set_state(self, text: str, mode: str, max_lines: int, align: int, anchor: int, gap: int,
                      line_colors: dict[int, tuple[int, int, int, int]] | None = None,
                      inline_style_plan: InlineStyleRenderPlan | None = None):
            self.text = text
            self.mode = mode
            ...
            self.line_colors = {int(key): tuple(value) for key, value in (line_colors or {}).items()}
            self.inline_style_plan = inline_style_plan
            self.update()
        ...
        def _stage_lines(self, n: int):
            ...
            if self.anchor == 0:
                ...
                y_last = self.STAGE_H - self.BOTTOM_OFF
            return x, [y_last - (n - 1 - i) * spacing for i in range(n)]

        def _runtime_segments_for_line(self, block: int, line: str):
            """Oculta controles consumidos pelo jogo e usa o plano do editor."""
            base = self.line_colors.get(block, (200, 239, 117, 255))
            plan = self.inline_style_plan
            if plan is None:
                return [(line, base)] if line else []
            spans = plan.spans.get(block, [])
            controls = plan.controls.get(block, [])
            def color_at(position: int):
                for start, length, rgba in spans:
                    if start <= position < start + length:
                        return rgba
                return base
            def append_range(out, start, end):
                if end <= start:
                    return
                points = {start, end}
                for span_start, length, _rgba in spans:
                ...
                for left, right in zip(ordered, ordered[1:]):
                    if right > left:
                        out.append((line[left:right], color_at(left)))
            result: list[tuple[str, tuple[int, int, int, int]]] = []
            position = 0
            for control in controls:
                append_range(result, position, control.start)
                position = control.end
            append_range(result, position, len(line))
            return result
        ...
        def paintEvent(self, _event):
            ...
            sx, sy = tv_w / self.STAGE_W, tv_h / self.STAGE_H
            ...
            if len(lines) > self.max_lines:
                painter.setPen(QColor("#e06a5f")); painter.setFont(QFont("Segoe UI", 8))
            ...
            start_index = max(0, len(lines) - self.max_lines)
            shown = list(enumerate(lines[start_index:], start_index))
            x, ys = self._stage_lines(len(shown))
            font_size = max(8, int(self.LINE_H * 0.72 * min(sx, sy)))
            painter.setFont(QFont("Consolas", font_size))
            for (block, line), yb in zip(shown, ys):
                box = QRectF(screen.left() + x * sx, screen.top() + (yb - self.LINE_H) * sy,
                             self._box_w() * sx, self.LINE_H * sy)
                painter.setPen(QPen(QColor("#426b4e"), 1, Qt.DashLine)); painter.drawRect(box)
                x_cursor = box.x() + 2
                for piece, rgba in self._runtime_segments_for_line(block, line):
                    painter.setPen(QColor(*rgba))
                    painter.drawText(QRectF(x_cursor, box.y() - 1, max(0.0, box.right() - x_cursor - 2), box.height()),
                                     Qt.AlignLeft | Qt.AlignVCenter, piece)
                    x_cursor += painter.fontMetrics().horizontalAdvance(piece)
            painter.setPen(QColor("#83ad8d")); painter.setFont(QFont("Segoe UI", 8))
            tag = ("4:3 · stage 800×600 medido no FLP · 5 linhas · passo 37.5px · caixa 440"
                   if self.ratio == 0 else
                   "16:9 · mod widescreen · mesmo stage 800×600 esticado · caixa 480")
            painter.drawText(QRectF(screen.left() + 8, screen.bottom() - 16, tv_w - 16, 12), Qt.AlignLeft, tag)


    class TextColorHighlighter(QSyntaxHighlighter):
        """Pintura WYSIWYG sem alterar o documento ou serializar HTML."""
        def __init__(self, document):
            super().__init__(document)
            self._line_colors: dict[int, tuple[int, int, int, int]] = {}
            self._inline_spans: dict[int, list[tuple[int, int, tuple[int, int, int, int]]]] = {}
            self._inline_controls: dict[int, list[InlineStyleControl]] = {}

        def set_line_colors(self, colors: dict[int, tuple[int, int, int, int]]):
            self.set_render_plan(colors, None)

        def set_render_plan(self, colors: dict[int, tuple[int, int, int, int]],
                            plan: InlineStyleRenderPlan | None = None):
            self._line_colors = {int(index): tuple(value) for index, value in colors.items()}
            self._inline_spans = {} if plan is None else {int(index): list(value) for index, value in plan.spans.items()}
            self._inline_controls = {} if plan is None else {int(index): list(value) for index, value in plan.controls.items()}
            # QTextDocument pode emitir contentsChanged durante rehighlight em
            # algumas versões Qt. Bloquear o documento evita a recursão de
            # on_editor_changed sem tocar no texto puro.
        ...
        @staticmethod
        def _format(rgba):
            fmt = QTextCharFormat(); fmt.setForeground(QColor(*rgba)); return fmt

        def highlightBlock(self, text: str):
            block = self.currentBlock().blockNumber()
            rgba = self._line_colors.get(block)
            if rgba is not None and text:
                self.setFormat(0, len(text), self._format(rgba))
            for start, length, span_rgba in self._inline_spans.get(block, []):
                if length:
                    self.setFormat(start, length, self._format(span_rgba))
            for control in self._inline_controls.get(block, []):
                fmt = QTextCharFormat()
                fmt.setForeground(QColor("#9aa7c5") if control.supported_by_renderer else QColor("#e06a5f"))
                fmt.setBackground(QColor("#252b3a"))
                fmt.setFontWeight(QFont.Weight.DemiBold)
                self.setFormat(control.start, control.end - control.start, fmt)
    ...
    class App(QMainWindow):
        def __init__(self):
            ...
            # Camada real de estilo Flash [*N] (independente das cores base R7).
            self.runtime_inline_palette = RuntimeInlinePalette.verified_shared_reference()
            self._inline_style_selected = 1
            self._inline_style_plan: InlineStyleRenderPlan | None = None
            self.dirty = False
        ...
        def _build_ui(self):
            ...
            self.editor_color_tag = QLabel("BASE COMPARTILHADA")
            ...
            self.editor_color_selector.setToolTip(
                "Camada base física de MessageTemplate_LineN. Pode haver mais de uma "
                "instância/estado; não substitui um [*N] que esteja sob o cursor."
            )
            ...
            self.editor_color_button = QPushButton("Editar base…")
            self.editor_color_button.setObjectName("MiniButton")
            self.editor_color_button.setEnabled(False)
            self.editor_color_button.setToolTip("Edita a base física compartilhada do DynamicLabel selecionado; veja o escopo antes de confirmar.")
            ...
            # Barra no próprio editor: tokens Flash permanecem texto bruto, mas
            # sua semântica/cor pode ser inserida e alterada sem abrir tabela.
            inline_style_bar = QWidget()
            inline_style_bar.setObjectName("PanelHeader")
            inline_style_hl = QHBoxLayout(inline_style_bar)
            inline_style_hl.setContentsMargins(int(8 * self._ui_scale), 1, int(8 * self._ui_scale), 1)
            inline_style_hl.setSpacing(int(4 * self._ui_scale))
            self.inline_style_tag = QLabel("ROTA DA SELEÇÃO")
            self.inline_style_tag.setObjectName("PanelTag")
            inline_style_hl.addWidget(self.inline_style_tag)
            self.inline_style_selector = QComboBox()
            self.inline_style_selector.setMinimumContentsLength(22)
            self.inline_style_selector.setSizeAdjustPolicy(QComboBox.AdjustToMinimumContentsLengthWithIcon)
            self.inline_style_selector.setToolTip(
                "A rota literal do renderer Flash. [*0]/[*] volta à camada base; "
                "[*1]..[*4] vale até o fim da mesma linha e usa FlashMsgNColor.")
            self.inline_style_selector.setEnabled(False)
            self.inline_style_selector.currentIndexChanged.connect(self._on_inline_style_selector_changed)
            inline_style_hl.addWidget(self.inline_style_selector)
            self.inline_style_apply_button = QPushButton("Aplicar à seleção")
            self.inline_style_apply_button.setObjectName("MiniButton")
            self.inline_style_apply_button.setToolTip(
                "Insere/troca um token serializável. Com seleção multilinha, usa um par por linha e preserva [Icon...].")
            self.inline_style_apply_button.setEnabled(False)
            self.inline_style_apply_button.clicked.connect(self._apply_inline_style_control)
            inline_style_hl.addWidget(self.inline_style_apply_button)
            self.inline_style_palette_button = QPushButton("Editar associada…")
            self.inline_style_palette_button.setObjectName("MiniButton")
            self.inline_style_palette_button.setToolTip(
                "Edita FlashMsgNColor, um slot compartilhado do runtime. A confirmação mostra quantas mensagens usam o slot.")
            self.inline_style_palette_button.setEnabled(False)
            self.inline_style_palette_button.clicked.connect(self._edit_inline_runtime_palette)
            inline_style_hl.addWidget(self.inline_style_palette_button)
            self.inline_style_exclusive_button = QPushButton("Nova cor exclusiva…")
            self.inline_style_exclusive_button.setObjectName("MiniButton")
            self.inline_style_exclusive_button.setToolTip(
                "Reserva um FlashMsgNColor hoje sem uso e envolve apenas a seleção. Há no máximo quatro slots runtime.")
            self.inline_style_exclusive_button.setEnabled(False)
            self.inline_style_exclusive_button.clicked.connect(self._apply_exclusive_inline_selection_color)
            inline_style_hl.addWidget(self.inline_style_exclusive_button)
            self.inline_style_hint = QLabel("Abra MSGS_TXT para inspecionar a rota runtime [*N].")
            self.inline_style_hint.setObjectName("PanelTag")
            self.inline_style_hint.setSizePolicy(QSizePolicy.Policy.Ignored, QSizePolicy.Policy.Fixed)
            inline_style_hl.addWidget(self.inline_style_hint, 1)
            editor_v.addWidget(inline_style_bar)
            ...
            try:
                ...
                with QSignalBlocker(self.codec_box):
                    if wad.variant.startswith("GoW2") and any(tag.upper_name == "MSGS_TXT" for tag in resources):
                        ...
                        self.codec_box.setCurrentText("Windows-1252 (browser/legado)")
                self.wad = wad
                # Ler a paleta logo na abertura: nunca usar um offset de string
                # presumido e nunca habilitar escrita sem GBL_Global físico.
                self.runtime_inline_palette = runtime_inline_palette_for_wad(wad)
                self.wad_path = path
        ...
        def on_editor_changed(self, *_args):
            ...
            # Mantém a pintura e a prévia sincronizadas sem alterar caracteres.
            self._refresh_editor_text_colors()
            self.update_preview()
        ...
        def update_preview(self, *_args):
            ...
            colors = getattr(self.editor_color_highlighter, "_line_colors", {}) if hasattr(self, "editor_color_highlighter") else {}
            self.preview.set_state(self._preview_text(), mode, max_lines,
                                   *self._placement_ids(), colors,
                                   getattr(self, "_inline_style_plan", None))
        ...
        # ---------------- rota runtime Flash [*N] (R9) ----------------

        def _is_inline_runtime_message_mode(self) -> bool:
            return bool(self.active_tag is not None and self.active_tag.upper_name == "MSGS_TXT"
                        and self.current_index is not None and not self._is_main_flp_mode())

        @staticmethod
        def _inline_rgb_hex(rgba: tuple[int, int, int, int] | None) -> str:
            return "#------" if rgba is None else "#%02X%02X%02X" % tuple(rgba[:3])

        def _selected_inline_style(self) -> int:
            value = self.inline_style_selector.currentData() if hasattr(self, "inline_style_selector") else None
            try:
                return int(value)
            except (TypeError, ValueError):
                return int(getattr(self, "_inline_style_selected", 1))

        def _inline_style_usage(self, style: int) -> dict:
            """Conta o escopo real de um FlashMsgNColor em MSGS_TXT carregados.

            Um ``[*N]`` fica no corpo de uma mensagem, mas o RGB pertence a
            ``GBL_Global.FlashMsgNColor``.  Mostrar só o token local induz a
            editar uma paleta compartilhada como se ela fosse exclusiva. A
            contagem percorre todas as tabelas MSGS_TXT do WAD e incorpora o
            texto ainda aberto no editor, antes mesmo de ele ser aplicado.
            """
            style = int(style)
            controls = 0
            message_refs: list[str] = []
            seen_messages: set[tuple[int, int]] = set()
            for tag in self.text_resources:
                if tag.upper_name != "MSGS_TXT":
                    continue
                try:
                    resource = self.active_resource if tag is self.active_tag else TextResource(
                        tag.data, self._codec_for_tag(tag)
                    )
                except Exception:
                    # Um recurso que o editor não consegue decodificar não
                    # habilita escrita; ele apenas não entra na estimativa.
                    continue
                if resource is None:
                    continue
                for index, message in enumerate(resource.messages):
                    value = message.text
                    if (tag is self.active_tag and index == self.current_index
                            and hasattr(self, "editor")):
                        value = self.editor.toPlainText()
                    count = sum(
                        1 for match in INLINE_STYLE_MARKER_RE.finditer(value)
                        if (0 if match.group("style") is None else int(match.group("style"))) == style
                    )
                    if not count:
                        continue
                    controls += count
                    ...
                    if key not in seen_messages:
                        ...
                        label = f"*{message.key}*"
                        if tag is not self.active_tag:
                            label = f"{tag.name}:{label}"
                        message_refs.append(label)
            return {
                "style": style,
                "controls": controls,
        ...
        def _inline_style_usages(self) -> dict[int, dict]:
            return {style: self._inline_style_usage(style) for style in INLINE_RENDERED_STYLE_IDS}

        def _inline_selection_routes(self) -> set[int]:
            if not hasattr(self, "editor"):
                return set()
            cursor = self.editor.textCursor()
            return inline_styles_in_range(
                self.editor.toPlainText(), cursor.selectionStart(), cursor.selectionEnd()
            )

        def _inline_route_note(self) -> tuple[set[int], str]:
            """Descreve a camada que chega ao cursor/seleção sem esconder estado."""
            if not self._is_inline_runtime_message_mode():
                return set(), "Abra uma mensagem MSGS_TXT para inspecionar [*N] no renderer Flash."
            routes = self._inline_selection_routes()
            if len(routes) != 1:
                displayed = ", ".join(f"[*{style}]" for style in sorted(routes)) or "nenhuma"
                return routes, (
                    f"SELEÇÃO MISTA ({displayed}): há mais de uma rota de cor. "
                    "Escolha um único trecho ou aplique um override inline à seleção."
                )
            style = next(iter(routes))
            if style in INLINE_RENDERED_STYLE_IDS:
                rgba = self.runtime_inline_palette.color(style)
                usage = self._inline_style_usage(style)
                return routes, (
                    f"EFETIVA: [*{style}] → FlashMsg{style}Color {self._inline_rgb_hex(rgba)}. "
                    f"ESCOPO COMPARTILHADO: {usage['controls']} marcador(es) em "
                    f"{usage['message_count']} mensagem(ns) MSGS_TXT."
                )
            if style != 0:
                return routes, (
                    f"[*{style}] está sob a seleção, mas o EditTextRender analisado só pinta "
                    "[*1]…[*4]; a tool não inventa uma cor para este token."
                )

            line = self._runtime_message_template_line_at_cursor()
            if line is None:
                return routes, (
                    "EFETIVA: [*0]/[*] → cor base da linha. Mova o cursor para uma linha "
                    "de conteúdo para identificar MessageTemplate_LineN e suas instâncias físicas."
                )
            candidates = self._message_template_direct_choices(line)
            if not candidates:
                return routes, (
                    f"EFETIVA: [*0]/[*] → MessageTemplate_Line{line}, mas nenhum DynamicLabel "
                    "físico correspondente foi localizado no FLP atual."
                )
            fields = []
            animations = 0
            for choice in candidates:
                field = choice["field"]
                paths = " / ".join(choice.get("runtime_paths", ())) or "ramo FLP sem rótulo"
                label = (
                    f"DynamicLabel [{field['dynamic_index']}] {FLPMovie.rgba_hex(field['rgba'])} "
                    f"via {paths}"
                )
                if label not in fields:
                    fields.append(label)
                animations += len(choice.get("animations", []))
            shown = "; ".join(fields[:4]) + ("; …" if len(fields) > 4 else "")
            animation_note = (
                f" {animations} BlendColor(es)/animação(ões) também alcançam a(s) instância(s)."
                if animations else ""
            )
            return routes, (
                f"BASE: [*0]/[*] → MessageTemplate_Line{line}. MessageTemplate_Style/estado "
                f"é escolhido pelo chamador no runtime; as instâncias físicas e seus ramos FLP são: {shown}. "
                f"ESCOPO COMPARTILHADO: toda mensagem que usa esta Line{line}.{animation_note}"
            )

        def _refresh_inline_style_controls(self, preferred_style: int | None = None):
            if not hasattr(self, "inline_style_selector"):
                return
            enabled = self._is_inline_runtime_message_mode()
            routes, route_note = self._inline_route_note() if enabled else (set(), "")
            if preferred_style is None:
                if len(routes) == 1 and next(iter(routes)) in range(0, 5):
                    preferred_style = next(iter(routes))
                else:
                    preferred_style = int(getattr(self, "_inline_style_selected", 1))
            palette = self.runtime_inline_palette
            usages = self._inline_style_usages() if enabled else {}
            with QSignalBlocker(self.inline_style_selector):
                self.inline_style_selector.clear()
                if enabled:
                    self.inline_style_selector.addItem("[*0] / [*] • base de MessageTemplate_LineN", 0)
                    for style in INLINE_RENDERED_STYLE_IDS:
                        rgba = palette.color(style)
                        usage = usages[style]
                        self.inline_style_selector.addItem(
                            f"[*{style}] • FlashMsg{style}Color • {self._inline_rgb_hex(rgba)} "
                            f"• {usage['controls']} uso(s) / {usage['message_count']} msg(s)", style)
                        row = self.inline_style_selector.count() - 1
                        if rgba is not None:
                            self.inline_style_selector.setItemData(row, QColor(*rgba), Qt.BackgroundRole)
                            light = rgba[0] * .2126 + rgba[1] * .7152 + rgba[2] * .0722
                            self.inline_style_selector.setItemData(
                                row, QColor("#111114") if light > 145 else QColor("#f2f2f6"), Qt.ForegroundRole
                            )
                    row = next((i for i in range(self.inline_style_selector.count())
                                if self.inline_style_selector.itemData(i) == preferred_style), 0)
                    self.inline_style_selector.setCurrentIndex(row)
            self._inline_style_selected = preferred_style
            self.inline_style_selector.setEnabled(enabled)
            self.inline_style_apply_button.setEnabled(enabled)
            self.inline_style_palette_button.setEnabled(
                enabled and preferred_style in INLINE_RENDERED_STYLE_IDS and palette.editable
            )
            has_selection = bool(enabled and self.editor.textCursor().hasSelection())
            self.inline_style_exclusive_button.setEnabled(enabled and palette.editable and has_selection)
            if not enabled:
                hint = "Abra uma mensagem MSGS_TXT para inspecionar a rota runtime [*N]."
            else:
                state = "RGB editável no Data Context" if palette.editable else "somente visualização neste WAD"
                hint = f"{route_note} • Fonte: {palette.source} ({state})."
            self.inline_style_hint.setText(hint)
            self.inline_style_hint.setToolTip(hint)

        def _on_inline_style_selector_changed(self, _index: int):
            if self._loading or not self._is_inline_runtime_message_mode():
                return
            self._inline_style_selected = self._selected_inline_style()
            # A lista pode ser escolhida para criar um override; o texto à
            # direita continua descrevendo a rota *atual* sob a seleção.
            self._refresh_inline_style_controls(self._inline_style_selected)

        def _sync_inline_style_from_cursor(self):
            if not self._is_inline_runtime_message_mode():
                return
            routes = self._inline_selection_routes()
            if len(routes) == 1:
                style = next(iter(routes))
                if style in range(0, 5):
                    self._inline_style_selected = style

        def _apply_inline_style_control(self):
            if not self._is_inline_runtime_message_mode():
                return
            style = self._selected_inline_style()
            cursor = self.editor.textCursor()
            text = self.editor.toPlainText()
            start, end = cursor.selectionStart(), cursor.selectionEnd()
            match = inline_style_control_at(text, cursor.position()) if start == end else None
            if match is not None:
                cursor.setPosition(match.start())
                cursor.setPosition(match.end(), QTextCursor.KeepAnchor)
                cursor.insertText(f"[*{style}]")
                status = f"Controle runtime {match.group(0)} alterado para [*{style}]."
            elif start != end:
                selected = text[start:end]
                rewritten = wrap_inline_style_selection(text, start, end, style)
                suffix_length = len(text) - end
                inserted_end = len(rewritten) - suffix_length if suffix_length else len(rewritten)
                cursor.insertText(rewritten[start:inserted_end])
                lines = len(re.split(r"\r\n|\r|\n", selected))
                status = (
                    f"Override [*{style}] aplicado só à seleção em {lines} linha(s); "
                    "a rota que havia depois do trecho foi restaurada."
                )
            else:
                cursor.insertText(f"[*{style}]")
                status = f"Controle [*{style}] inserido no cursor."
            self._set_status(status)
            self._refresh_editor_text_colors()
            self.update_preview()

        def _edit_inline_runtime_palette(self):
            style = self._selected_inline_style()
            palette = self.runtime_inline_palette
            if style not in INLINE_RENDERED_STYLE_IDS:
                QMessageBox.information(self, "Cor associada", "[*0] e [*] usam a camada base; escolha [*1] até [*4].")
                return
            if not palette.editable or self.wad is None or palette.data_tag is None:
                QMessageBox.information(self, "Cor associada", "Este WAD não contém GBL_Global editável para a paleta [*N].")
                return
            rgba = palette.color(style)
            if rgba is None:
                QMessageBox.warning(self, "Cor associada", "FlashMsgColor não foi encontrada.")
                return
            usage = self._inline_style_usage(style)
            scope = (
                f"FlashMsg{style}Color é um slot GLOBAL do runtime. Há "
                f"{usage['controls']} marcador(es) [*{style}] em {usage['message_count']} mensagem(ns) "
                "MSGS_TXT carregadas. Alterar o RGB muda todas elas; não é uma edição exclusiva da seleção."
            )
            answer = QMessageBox.question(
                self, "Editar cor associada compartilhada?", scope + "\n\nContinuar?",
                QMessageBox.Yes | QMessageBox.No, QMessageBox.No,
            )
            if answer != QMessageBox.Yes:
                return
            chosen = QColorDialog.getColor(QColor(*rgba), self,
                                            f"Editar FlashMsg{style}Color compartilhada (runtime [*{style}])")
            if not chosen.isValid():
                return
            wanted = (chosen.red(), chosen.green(), chosen.blue())
            if wanted == tuple(rgba[:3]):
                return
            try:
                updated = patched_runtime_inline_palette_rgb(palette, style, wanted)
                target = palette.data_tag
                if len(updated) != len(target.data):
                    raise ValueError("A paleta inline não pode alterar o tamanho do Data Context.")
                target.data = updated
                new_palette = find_runtime_inline_palette(self.wad)
                if new_palette is None:
                    raise ValueError("Não foi possível reler GBL_Global após o patch.")
                self.runtime_inline_palette = new_palette
                self.dirty = True
                self._refresh_dirty()
                self._refresh_editor_text_colors()
                self._refresh_inline_style_controls(style)
                self.update_preview()
                self._set_status(
                    f"FlashMsg{style}Color = #{wanted[0]:02X}{wanted[1]:02X}{wanted[2]:02X}; "
                    f"{usage['message_count']} mensagem(ns) compartilham a alteração."
                )
            except Exception as exc:
                QMessageBox.critical(self, "Não foi possível editar a cor associada", str(exc))

        def _apply_exclusive_inline_selection_color(self):
            """Reserva um slot Flash livre e o aplica somente ao texto selecionado."""
            if not self._is_inline_runtime_message_mode():
                return
            cursor = self.editor.textCursor()
            if not cursor.hasSelection():
                QMessageBox.information(self, "Nova cor exclusiva", "Selecione o trecho que deve receber o override inline.")
                return
            text = self.editor.toPlainText()
            start, end = cursor.selectionStart(), cursor.selectionEnd()
            if not text[start:end].replace("\r", "").replace("\n", ""):
                QMessageBox.information(self, "Nova cor exclusiva", "A seleção não contém texto para pintar.")
                return
            palette = self.runtime_inline_palette
            if not palette.editable or self.wad is None or palette.data_tag is None:
                QMessageBox.information(self, "Nova cor exclusiva", "Este WAD não contém GBL_Global editável para criar o override.")
                return
            usages = self._inline_style_usages()
            free = [style for style in INLINE_RENDERED_STYLE_IDS if usages[style]["controls"] == 0]
            if not free:
                QMessageBox.information(
                    self, "Nova cor exclusiva",
                    "Os quatro slots que o EditTextRender comprovadamente desenha ([*1]…[*4]) já têm uso. "
                    "Para não mudar outras mensagens, escolha uma cor existente e aplique apenas seu token à seleção. "
                    "RGB arbitrário adicional exigiria alterar o runtime/ELF, não apenas o MSGS_TXT.",
                )
                return
            style = free[0]
            rgba = palette.color(style)
            if rgba is None:
                QMessageBox.warning(self, "Nova cor exclusiva", f"FlashMsg{style}Color não foi encontrada.")
                return
            chosen = QColorDialog.getColor(
                QColor(*rgba), self, f"Nova cor exclusiva: reservar FlashMsg{style}Color"
            )
            if not chosen.isValid():
                return
            wanted = (chosen.red(), chosen.green(), chosen.blue())
            answer = QMessageBox.question(
                self, "Reservar slot para esta seleção?",
                f"FlashMsg{style}Color não possui [*{style}] em nenhuma mensagem MSGS_TXT carregada.\n\n"
                f"A tool gravará o RGB #{wanted[0]:02X}{wanted[1]:02X}{wanted[2]:02X} nesse slot e envolverá "
                "somente a seleção por tokens [*N]. As demais mensagens e suas cores atuais não serão alteradas.\n\n"
                "O renderer tem somente quatro slots desse tipo. Continuar?",
                QMessageBox.Yes | QMessageBox.No, QMessageBox.No,
            )
            if answer != QMessageBox.Yes:
                return
            try:
                ...
                new_palette = find_runtime_inline_palette(self.wad)
                if new_palette is None:
                    raise ValueError("Não foi possível reler GBL_Global após reservar o slot.")
                self.runtime_inline_palette = new_palette
                # O helper restaura o estilo original depois do trecho: nem o
                # texto seguinte da mesma linha é afetado pelo override novo.
                rewritten = wrap_inline_style_selection(text, start, end, style)
                suffix_length = len(text) - end
                inserted_end = len(rewritten) - suffix_length if suffix_length else len(rewritten)
                cursor.insertText(rewritten[start:inserted_end])
                ...
                self.update_preview()
                self._set_status(
                    f"Cor exclusiva #{wanted[0]:02X}{wanted[1]:02X}{wanted[2]:02X} reservada em FlashMsg{style}Color "
                    "e aplicada somente à seleção; salve o WAD para gravar."
                )
            except Exception as exc:
                QMessageBox.critical(self, "Não foi possível criar a cor exclusiva", str(exc))
        ...
        def _clear_editor_text_colors(self, reason: str = ""):
            ...
            if not hasattr(self, "editor_color_highlighter"):
                return
            self.editor_color_highlighter.set_render_plan({}, None)
            self._inline_style_plan = None
            ...
            self.editor_color_selector.setEnabled(False)
            self.editor_color_button.setEnabled(False)
            self.editor_color_button.setText("Editar base…")
            self.editor_color_button.setStyleSheet("")
            self.editor_color_tag.setText("BASE COMPARTILHADA")
            ...
            for tag, movie in self.main_flp_resources:
                ...
                for target in targets:
                    ...
                    if not fields:
                        continue
                    field = fields[0]
                    paths = tuple(target["colors"].get("runtime_paths", ()))
                    route = " / ".join(paths) if paths else "ramo FLP sem rótulo"
                    choice = self._make_editor_color_choice(
                        tag, movie, field, "message", line,
                        target["colors"].get("animations", []),
                        f"Linha {line} • DynamicLabel [{field['dynamic_index']}] • {route}",
                    )
                    # A rota é derivada dos KeyFrames até Data8. Ela distingue
                    # instâncias duplicadas sem alegar que a variável externa
                    # MessageTemplate_Style tem um valor estático no TXT.
                    choice["runtime_paths"] = paths
        ...
        def _apply_editor_color_button(self, choice: dict | None):
            ...
            if choice.get("role") == "message":
                scope = "BASE COMPARTILHADA de MessageTemplate_LineN/estado"
            else:
                scope = "Cor física direta"
            if count:
                note = f"{scope} • {count} BlendColor(es)/animação(ões) também alcançam este texto"
            else:
                note = f"{scope} • sem BlendColor de texto mapeada para este campo"
            self.editor_animation_hint.setText(note)
            self.editor_animation_hint.setToolTip(
                note + ". A cor animada varia por KeyFrame. Um [*N] sob o cursor substitui esta base."
            )
        ...
        def _refresh_editor_text_colors(self, keep_token: str | None = None):
            ...
            inline_plan = None
            if self.active_tag.upper_name == "MSGS_TXT":
                inline_plan = build_inline_style_render_plan(self.editor.toPlainText(), self.runtime_inline_palette)
            self._inline_style_plan = inline_plan
            self.editor_color_highlighter.set_render_plan(line_colors, inline_plan)
            ...
            self.editor_color_tag.setText("BASE COMPARTILHADA" if current_choices else "SEM COR FLP")
            self._apply_editor_color_button(selected)
            # Um [*N] é uma rota de runtime mais específica que a base do
            # DynamicLabel. Não deixar o botão de base parecer a cor do título
            # ou de outro trecho inline evita modificar um template global por
            # engano; basta mover o cursor para texto [*0] para reabilitá-lo.
            selection_routes = self._inline_selection_routes() if self._is_inline_runtime_message_mode() else set()
            inline_route = bool(selection_routes.intersection(INLINE_RENDERED_STYLE_IDS))
            if inline_route:
                self.editor_color_selector.setEnabled(False)
                self.editor_color_button.setEnabled(False)
                self.editor_color_tag.setText("BASE NÃO PINTA [*N]")
                base_note = (
                    "A seleção usa FlashMsgNColor; edite a rota associada na barra abaixo. "
                    "Esta base física é compartilhada e não é a cor final deste trecho."
                )
                self.editor_animation_hint.setText(base_note)
                self.editor_animation_hint.setToolTip(base_note)
            elif not current_choices:
                self.editor_animation_hint.setText(description)
                self.editor_animation_hint.setToolTip(description)
            self._refresh_inline_style_controls()
        ...
        def _edit_editor_color_field(self):
            ...
            try:
                ...
                if choice.get("role") == "message":
                    scope_note = (
                        f"{choice['title']} é uma base física compartilhada do MessageTemplate. "
                        "Ela pode ser usada por muitas mensagens e não muda um trecho coberto por [*N].\n\n"
                        "Para a cor efetiva de um [*N], use ‘Editar associada…’ na barra ROTA DA SELEÇÃO.\n\n"
                        "Editar esta base mesmo assim?"
                    )
                    answer = QMessageBox.question(
                        self, "Editar base compartilhada?", scope_note,
                        QMessageBox.Yes | QMessageBox.No, QMessageBox.No,
                    )
                    if answer != QMessageBox.Yes:
                        return
        ...
        def _on_editor_cursor_changed(self):
            ...
            self._sync_inline_style_from_cursor()
            self._refresh_editor_text_colors()
            self.update_preview()
